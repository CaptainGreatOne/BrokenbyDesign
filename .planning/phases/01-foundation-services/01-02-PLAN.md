...
files_modified:
  ...
  - services/order-api/proto/order_pb2.py
  - services/order-api/proto/order_pb2_grpc.py
autonomous: true
...
<task type="auto">
  ...
  <action>
    ...
    4. **services/order-api/Dockerfile**:
       ...
       - Note: The build context is services/order-api/, but we need access to proto/ from the repo root. Two approaches:
         a) Set build context to repo root in docker-compose.yml: `context: .` with `dockerfile: services/order-api/Dockerfile`
         b) Copy proto file into service dir during build.
       - Use approach (a): In docker-compose.yml, the order-api service should use `build: { context: ., dockerfile: services/order-api/Dockerfile }`. Update docker-compose.yml if Plan 01 used `build: ./services/order-api`. This is acceptable since Plan 01's SUMMARY will document the compose structure and this plan explicitly owns the order-api build configuration.
  </action>
  <verify>
