...
<task type="auto">
  ...
  <action>
    ...
    1. **docker-compose.yml** - Full service orchestration (no version field -- modern Compose v2 doesn't need it):
       ...
       **IMPORTANT BUILD CONTEXT SPECIFICATIONS:**
       - order-api service: Use `build: { context: ., dockerfile: services/order-api/Dockerfile }` to set build context to repo root (required for proto/ access)
       - web-gateway service: Use `build: { context: ., dockerfile: services/web-gateway/Dockerfile }` to set build context to repo root (required for proto/ access)
  ...
  <verify>
    ...
    - docker-compose.yml specifies `build: { context: ., dockerfile: ... }` for order-api and web-gateway services
  </verify>
  <done>
    docker-compose.yml defines 7 services (postgres, redis, nginx, web-gateway, order-api, fulfillment-worker, traffic-generator) with health checks and dependency ordering. Build contexts for order-api and web-gateway are set to repo root (.) with explicit dockerfile paths for proto/ access. Makefile provides shortcuts for all common operations. .gitignore prevents committing secrets and generated files.
  </done>
</task>
