...
NOTE on METR-02 scope: The requirement METR-02 mentions Loki and Jaeger datasources, but those are out of scope for Phase 2. Only Prometheus is provisioned now. The provisioning directory structure (`grafana/provisioning/datasources/`) is designed so that Loki datasource can be added in Phase 3 (Centralized Logging) and Jaeger datasource in Phase 5 (Distributed Tracing) by simply adding additional YAML files to the same directory.
</objective>
...
2. Create `grafana/provisioning/datasources/prometheus.yml`:
   ...
   NOTE: Only Prometheus is provisioned in Phase 2. Additional datasources (Loki in Phase 3, Jaeger in Phase 5) will be added as separate YAML files in this same directory. Grafana auto-discovers all YAML files in the provisioning/datasources/ directory.
...
Run `docker compose config` to validate. Verify grafana/provisioning/datasources/prometheus.yml exists. Verify grafana/provisioning/dashboards/dashboards.yml exists. Verify docker-compose.yml has grafana service with correct volume mounts and depends_on.
  ...
  <done>
Grafana provisioning directory structure created. Prometheus datasource auto-configured. Dashboard provider points to /var/lib/grafana/dashboards. Grafana service in docker-compose.yml with healthcheck, depends_on prometheus (service_healthy), port 3001, resource limits, and provisioning volume mounts. Provisioning structure supports future Loki (Phase 3) and Jaeger (Phase 5) datasource additions.
  </done>
</task>
...
Validate each JSON file is valid JSON: `python3 -c "import json; json.load(open('grafana/dashboards/service-overview.json'))"` (and same for the other two). Verify each dashboard has panels with PromQL expressions. Verify the provisioning datasource YAML has `uid: prometheus` matching the dashboard datasource references. Run `docker compose config` to validate the full compose file.
  ...
  <done>
Three dashboard JSON files created and valid. Service Overview shows request rates, error rates, p95/p99 latency across all services. Container Resources shows per-container CPU, memory, and network from cAdvisor. Host Metrics shows host CPU, memory, disk, and I/O from Node Exporter. Grafana datasource provisioning includes uid for dashboard references. All dashboards have auto-refresh 10s and 1-hour time range. Provisioning structure ready for Loki (Phase 3) and Jaeger (Phase 5) datasources to be added later.
  </done>
</task>
...
<success_criteria>
- Grafana auto-provisions Prometheus datasource on first boot (Loki/Jaeger datasources deferred to Phases 3/5)
